...
class HLSImagePipelineComposer(Composer):
    ...
    def _render_hls_config(self, hls_design: Mapping[str, Any]) -> str:
        ...
        if self.vitis_version == "2023.1":
            config = self._remove_hls_config_values(
                config,
                ("package.output.format", "package.output.syn"),
            )
            replacements["syn.output.format"] = "rtl"
        else:
            replacements["package.output.format"] = "rtl"
            replacements["package.output.syn"] = "false"
        return self._replace_hls_config_values(config, replacements)

    @staticmethod
    def _remove_hls_config_values(config: str, keys: Sequence[str]) -> str:
        prefixes = tuple(f"{key}=" for key in keys)
        return "\n".join(
            line for line in config.splitlines() if not line.strip().startswith(prefixes)
        )
    ...
    def _template_name(self) -> str:
        ...
        if self.interface == "safa_fifo":
            return "safa_fifo_top.cpp.tpl"
        if self.interface == "axi_stream":
