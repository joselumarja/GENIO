...
def test_hls_image_pipeline_composer_generates_fifo_package():
    ...
    assert "package.output.format=rtl" in package.files["hls_config.cfg"]
    assert "package.output.syn=false" in package.files["hls_config.cfg"]
    assert "vivado.flow" not in package.files["hls_config.cfg"]
    assert package.files["hls_config.cfg"].count("syn.op=") == 24
...
def test_hls_image_pipeline_composer_generates_safa_fifo_package():
    composer = HLSImagePipelineComposer(
        DEFINITIONS_PATH,
        templates_path=ROOT / "hls_templates/vitis_vision_image_pipeline",
        interface="safa_fifo",
    ...
    assert package.metadata["interface"] == "safa_fifo"
    assert package.metadata["top_function"] == "safa_accelerator"
    assert "void safa_accelerator(" in source
    ...
    assert "#define OUTPUT_WORDS ((48 * 48) / XF_NPIXPERCYCLE(XF_NPPC1))" in source
    assert "xf::cv::bgr2gray<XF_8UC3, XF_8UC1, 48, 48, XF_NPPC1>" in source
...
def test_hls_image_pipeline_composer_uses_2023_resize_signature():
    ...
    assert "syn.output.format=rtl" in package.files["hls_config.cfg"]
    assert "package.output.format" not in package.files["hls_config.cfg"]
    assert "package.output.syn" not in package.files["hls_config.cfg"]
    assert "vivado.flow" not in package.files["hls_config.cfg"]
...
def test_hls_image_pipeline_composer_renders_insect_segmentation_sample():
    ...
    assert "unsigned char kernel[" not in source
    assert "Local kernel declaration generated by composer" not in source
    assert "kernel_4);" in source
...
def test_hls_image_pipeline_composer_generates_unique_morphology_kernels():
    parameters = {
        "kernel_shape": "rect",
        "kernel_rows": 3,
        "kernel_cols": 5,
    ...
    individual = Individual.from_slots(
        ...
        slots=[
            ...
            StageChoice(slot=3, stage="dilate", parameters=parameters),
            StageChoice(slot=4, stage="erode", parameters=parameters),
        ],
    )
    composer = HLSImagePipelineComposer(
        DEFINITIONS_PATH,
    ...
    source = composer.compose(individual).files["src/pipeline.cpp"]

    assert source.count("unsigned char kernel_") == 4
    assert "unsigned char kernel[" not in source
    assert "Local kernel declaration generated by composer" not in source
    assert "imgAux" not in source
    for stage_index in range(1, 5):
        assert f"unsigned char kernel_{stage_index}[15]" in source
        assert f"kernel_{stage_index});" in source
    assert "img_aux_1" in source
    assert "img_aux_2" in source
